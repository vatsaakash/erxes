...
import CategoryForm from '../../containers/contractCategory/CategoryForm';
...
interface IProps {
  ...
  queryParams: any;
  refetch: any;
  remove: (contractCategoryId: string) => void;
...
class List extends React.Component<IProps> {
  renderFormTrigger(trigger: React.ReactNode, category?: IContractCategory) {
    const content = props => (
      <CategoryForm
        {...props}
        category={category}
        categories={this.props.contractCategories}
      />
    );
  ...
  renderEditAction(category: IContractCategory) {
    const trigger = (
  ...
  renderRemoveAction(category: IContractCategory) {
  ...
  render() {
    return (
      <Sidebar>
        <Section
          maxHeight={188}
          collapsible={this.props.contractCategoriesCount > 30}
        >
