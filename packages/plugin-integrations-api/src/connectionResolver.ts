...
export const loadClasses = (db: mongoose.Connection) => {
  ...
  models.FbCustomers = db.model<IFbCustomerDocument, IFbCustomerModel>(
    'customers_facebook',
    loadFbCustomerClass(models)
  );
  models.FbConversations = db.model<
    IFbConversationDocument,
    IFbConversationModel
  >('conversations_facebook', loadFbConversationClass(models));
  models.FbConversationMessages = db.model<
    IFbConversationMessageDocument,
    IFbConversationMessageModel
  >('conversation_messages_facebook', fbConversationMessageSchema);
  models.FbPosts = db.model<IFbPostDocument, IFbPostModel>(
    'posts_facebook',
    loadfbPostClass(models)
  );
  models.FbComments = db.model<IFbCommentDocument, IFbCommentModel>(
    'comments_facebook',
