...
const TypesListContainer = (props: FinalProps) => {
  const { listNoteTypeQuery, typesEdit, typesRemove, history } = props;

  if (listNoteTypeQuery.loading) {
    return <Spinner />;
  }
  ...
  const renderButton = ({
    ...
    return (
      <ButtonMutate
        ...
        refetchQueries={['listNoteTypeQuery']}
      />
    );
  };
  ...
  const updatedProps = {
    ...
    loading: listNoteTypeQuery.loading,
    remove,
    renderButton
  };
