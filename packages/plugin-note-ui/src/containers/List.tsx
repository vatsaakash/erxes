...
type FinalProps = {
  listQuery: NoteQueryResponse;
  listNoteTypeQuery: TypeQueryResponse;
} & Props &
  RemoveMutationResponse &
  EditMutationResponse;
...
const ListContainer = (props: FinalProps) => {
  ...
  const types = listNoteTypeQuery.noteTypes || [];
  ...
  const remove = note => {
    confirm('You are about to delete the item. Are you sure? ')
      .then(() => {
        removeMutation({ variables: { _id: note._id } })
          .then(() => {
  ...
  const updatedProps = {
    ...props,
    notes: listQuery.notes || [],
    types: listNoteTypeQuery.noteTypes || [],
    typeId,
...
export default withProps<Props>(
  compose(
    graphql(gql(queries.listNoteTypes), {
      name: 'listNoteTypeQuery',
      options: () => ({
        fetchPolicy: 'network-only'
      })
    }),

    graphql<Props, NoteQueryResponse, { typeId: string }>(gql(queries.list), {
      name: 'listQuery',
      options: ({ typeId }) => ({
