...
const getToChangeObjects = async ({ triggerType, target, module }) => {
  ...
  if (
    ...
    ['task', 'ticket', 'deal'].includes(module)
  ) {
    return sendRPCMessage('find-objects', {
      model: `${module[0].toUpperCase()}${module.substr(1)}s`,
      selector: { sourceConversationIds: { $in: [target._id] } }
    });
  }

  if (
    ['customer', 'company'].includes(module) &&
    triggerType === 'conversation'
  ) {
    return sendRPCMessage('find-objects', {
      model: module === 'company' ? 'Companies' : 'Customers',
      selector: { _id: target[`${module}Id`] }
    });
  }

  if (
    ['task', 'ticket', 'deal', 'customer', 'company'].includes(triggerType) &&
    ['task', 'ticket', 'deal', 'customer', 'company'].includes(module)
  ) {
    return sendRPCMessage('find-conformities', {
      mainType: triggerType,
...
export const setProperty = async ({ triggerType, actionConfig, target }) => {
  ...
  try {
    const conformities = await getToChangeObjects({
      triggerType,
      target,
      module
    });
    ...
    for (const conformity of conformities) {
      ...
      const response = await sendRPCMessage('set-property', {
        model: modelBy[module],
        _id: conformity._id,
        setDoc
      });
      if (response.error) {
